...
# with restricted treatment eligibility.
#
# Relative to BFT_addhealth_skeleton.R, the only design change is:
# treatment may be assigned only to a fixed subset of eligible nodes.
#
# In this implementation, eligible nodes are the highest-degree nodes in the
# network, with the eligible set size determined by eligible_frac.
...
select_eligible_nodes <- function(A, n_treated, eligible_frac = 0.25) {
  N <- nrow(A)
  degree <- rowSums(A)
  n_eligible <- max(n_treated, ceiling(N * eligible_frac))

  rank_order <- order(degree, decreasing = TRUE, seq_len(N))
  rank_order[seq_len(n_eligible)]
}

draw_restricted_randomization <- function(N, n_treated, eligible_idx) {
  Z <- rep(0, N)
  treated_idx <- sample(eligible_idx, size = n_treated, replace = FALSE)
  Z[treated_idx] <- 1
  Z
}
...
build_bft_hypothesis_restricted <- function(A, n_treated, eligible_idx) {
  list(
    design_fn = function() {
      draw_restricted_randomization(nrow(A), n_treated, eligible_idx)
    },
...
run_bft_addhealth_restricted <- function(comm_name = "comm_82",
                                         treated_frac = 0.10,
  ...
  eligible_idx <- select_eligible_nodes(A, n_treated, eligible_frac = eligible_frac)
  Z_obs <- draw_restricted_randomization(N, n_treated, eligible_idx)
  ...
  hypothesis <- build_bft_hypothesis_restricted(A, n_treated, eligible_idx)
  controls <- list(
    method = "greedy",
    mina = mina,
...
summarize_bft_restricted_result <- function(result) {
  ...
  summary_df <- data.frame(
    ...
    n_eligible = result$settings$n_eligible,
    eligible_frac = result$settings$eligible_frac,
...
benchmark_bft_communities_restricted <- function(comm_names,
                                                 treated_frac = 0.10,
  ...
  for (idx in seq_along(comm_names)) {
    ...
    if (verbose) {
      message(
        sprintf(
          "[%d/%d] Restricted benchmark %s | seed=%d | R=%d",
    ...
    summary_row <- summarize_bft_restricted_result(run_out)
...
select_best_benchmark_communities_restricted <- function(benchmark_table, top_k = 5) {
...
rerun_selected_benchmark_communities_restricted <- function(selected_table,
                                                            treated_frac = 0.10,
  ...
  benchmark_bft_communities_restricted(
...
run_bft_benchmark_workflow_restricted <- function(
    benchmark_comm_names = names(addhealth),
    top_k = 5,
    treated_frac = 0.10,
    ...
    eligible_frac = 0.25,
    verbose = TRUE) {
  benchmark_out <- benchmark_bft_communities_restricted(
  ...
  selected_table <- select_best_benchmark_communities_restricted(
    benchmark_out$benchmark_table,
    top_k = top_k
  )

  rerun_out <- rerun_selected_benchmark_communities_restricted(
...
# Example single run:
# out <- run_bft_addhealth_restricted(
#   comm_name = "comm_82",
#   num_randomizations = 1000,
#   eligible_frac = 0.25
# )
# summarize_bft_restricted_result(out)
#
# Example full workflow:
# workflow_out <- run_bft_benchmark_workflow_restricted(
#   benchmark_comm_names = names(addhealth),
#   top_k = 5,
#   benchmark_num_randomizations = 200,
#   rerun_num_randomizations = 1000,
